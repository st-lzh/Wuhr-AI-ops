...
const GitCredentialManager: React.FC<GitCredentialManagerProps> = ({
    ...
    <>
      <div style={{ marginBottom: 16 }}>
        <Space>
          ...
          <Button icon={<SafetyOutlined />} onClick={loadCredentials} loading={loading}>
            刷新
          </Button>
        </Space>
      </div>
      ...
      <Alert
        message="认证配置说明"
        description="认证配置用于访问私有 Git 仓库。密钥只加密保存且不会回显；需要更换密钥时，请删除旧配置后重新创建。"
        type="info"
        showIcon
  ...
  return (
    <>
      {embedded ? (
        <Card title={<Space><SafetyOutlined />Git 认证配置</Space>}>
          {managerContent}
        </Card>
      ) : (
        <Modal
          title={<Space><SafetyOutlined />Git 认证配置管理</Space>}
          open={visible}
          onCancel={onCancel}
          width={800}
